...
public class WallapopServiceTemporal {
    private WebDriver driver;
    private final ItemDAOImplementation itemImp = new ItemDAOImplementation();
    private final WallapopUploadPage wallaUpload;

    public WallapopServiceTemporal(){
        //element wait time
    ...
    private void detectedChromeInstance(){
        ...
        try {
            ...
            Process process = Runtime.getRuntime().exec("task list");
            BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()));
            // kill chrome if found
            String line;
            while ((line = reader.readLine()) != null) {
                if (line.toLowerCase().contains(processName.toLowerCase())) {
                    Runtime.getRuntime().exec("task kill /F /IM chrome.exe");
                    break;
                }
            }
        } catch (IOException e) {
